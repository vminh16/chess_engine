...
class Report1TrainConfig:
    run_name: str = "dgrn_5m_report1_r1_shrink_run1"
    phase_name: str = "R1"
    epochs: int = 15
    sampling_mode: str = "band_balanced"
    main_batch_size: int = 768
    clean_center_batch_size: int = 96
    ambiguous_center_batch_size: int = 192
...
def _restore_rng_state(state: object) -> None:
    ...
    try:
        ...
        print(f"[report1][resume] Failed to restore RNG state ({exc}); continuing with seeded RNG")
...
def _torch_load_trusted_checkpoint(path: str | Path, map_location: str | torch.device = "cpu") -> object:
    ...
    Report1/Phase-B checkpoints store Python and NumPy RNG states for exact resume.
...
def validate_report1_train_config(train_cfg: Report1TrainConfig) -> None:
    ...
    if sampling_mode not in {"band_balanced", "sign_stratified"}:
        raise ValueError("train_cfg.sampling_mode must be one of: band_balanced, sign_stratified")
    ...
    try:
        for step in range(total_steps):
            ...
            with autocast(device_type=device.type, enabled=amp_enabled, dtype=amp_dtype if amp_enabled else None):
                ...
                main_terms = shared_ft1.compute_l4_main_terms(main_logits, yb_main, variant, train_cfg)
                aux_terms = shared_ft1.compute_ft1_aux_terms(aux_logits, yb_aux, role_aux, train_cfg)
    ...
    if resume_scheduler_state is not None:
        ...
        if expected_scheduler_t_max is not None:
            ...
            if saved_t_max != int(expected_scheduler_t_max):
                print(
                    "[report1][resume] Updating scheduler T_max from checkpoint "
...
def run_report1_training(
    ...
    if not bool(runtime_check["ok"]):
        raise RuntimeError("Report1 runtime validation failed: " + json.dumps(runtime_check["missing"], ensure_ascii=False))
    ...
    if existing_run_config is not None and isinstance(existing_run_config.get("train_cfg"), dict):
        ...
        print(
            "[report1][resume] Reusing saved train_cfg/profile from existing run_config; "
    ...
    if latest_ckpt.exists() and step_history_path.exists():
        try:
            ...
            print(f"[report1][resume] Failed to load existing step_history.csv ({exc}); starting step log from this session")
            step_rows = []
    else:
        step_rows = []
    ...
    if isinstance(resume["resume_aux_rng_state"], dict):
        try:
            ...
            print(f"[report1][resume] Failed to restore aux RNG state ({exc}); continuing with seeded RNG")
    ...
    for epoch in range(int(resume["start_epoch"]), int(tuned_cfg.epochs)):
        ...
        for shard_index, pack in enumerate(prefetcher):
            ...
            for start in range(shard_start_offset, int(order.shape[0]), int(tuned_cfg.main_batch_size)):
                ...
                with autocast(device_type=device.type, enabled=amp_enabled, dtype=amp_dtype if amp_enabled else None):
                    ...
                    main_logits = logits_mix[: yb_main.numel()]
                    aux_logits = logits_mix[yb_main.numel() :]
                    main_terms = shared_ft1.compute_l4_main_terms(main_logits, yb_main, variant, tuned_cfg)
                    aux_terms = shared_ft1.compute_ft1_aux_terms(aux_logits, yb_aux, role_aux, tuned_cfg)
                ...
                if not bool(torch.isfinite(total_objective)):
                    ...
                    accum_micro_step = 0
                    print(
                        f"[report1][epoch={epoch}] non-finite objective at shard={pack['shard_id']}, "
                ...
                if not _all_grads_finite(all_params):
                    ...
                    accum_micro_step = 0
                    print(
                        f"[report1][epoch={epoch}] non-finite accumulated gradients at shard={pack['shard_id']}; "
                        "dropping current accumulation window"
                    )
                    continue
                ...
                if should_step:
                    if not _all_grads_finite(all_params):
                        ...
                        accum_micro_step = 0
                        print(
                            f"[report1][epoch={epoch}] non-finite gradients before step={next_global_step}; "
                            "dropping current accumulation window"
                        )
                        continue
                    ...
                    if global_step % max(int(tuned_cfg.log_every_steps), 1) == 0:
                        print(
                            f"[report1][epoch={epoch}] step={global_step} "
        ...
        if accum_micro_step != 0:
            if not _all_grads_finite(all_params):
                optimizer.zero_grad(set_to_none=True)
                print(
                    f"[report1][epoch={epoch}] non-finite gradients at tail step={global_step + 1}; "
        ...
        print(
            f"[report1][epoch={epoch}] phase={tuned_cfg.phase_name} "
